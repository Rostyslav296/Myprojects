...
def sha256_detailed(data: bytes, verbose: bool = False, expanded: bool = False) -> bytes:
    ...
    if expanded:
        print(f"{Colors.CYAN}Math: Initial H values are H_0 = \\sqrt{2}, H_1 = \\sqrt{3}, ..., H_7 = \\sqrt{23} (fractional parts, 32 bits). K constants are \\sqrt[3]{2}, \\sqrt[3]{3}, ..., \\sqrt[3]{71} (fractional parts, 32 bits).{Colors.RESET}")
    ...
    if expanded:
        print(f"{Colors.CYAN}Math: Padding: Append 1 (0x80), then k zeros where k = (448 - (l + 1)) mod 512, then 64-bit big-endian length l. Ensures length ≡ 0 mod 512.{Colors.RESET}")
    ...
    for i in range(0, len(data), 64):
        ...
        if expanded:
            print(f"{Colors.CYAN}Math: For j=16 to 63: w_j = w_{j-16} + \\sigma_0(w_{j-15}) + w_{j-7} + \\sigma_1(w_{j-2}), where \\sigma_0(X) = ROTR^7(X) \\oplus ROTR^{18}(X) \\oplus SHR^3(X), \\sigma_1(X) = ROTR^{17}(X) \\oplus ROTR^{19}(X) \\oplus SHR^{10}(X). All mod 2^{32}.{Colors.RESET}")

        a, b, c, d, e, f, g, h = H

        if expanded:
            print(f"{Colors.CYAN}Math: Initialize a=H_0, b=H_1, ..., h=H_7.{Colors.RESET}")
        ...
        for j in range(64):
            ...
            if expanded:
                ...
                print(f"{Colors.CYAN}Math: \\Sigma_0(a) = ROTR^2(a) \\oplus ROTR^{13}(a) \\oplus ROTR^{22}(a), Maj(a,b,c) = (a \\land b) \\oplus (a \\land c) \\oplus (b \\land c), temp2 = \\Sigma_0(a) + Maj(a,b,c) mod 2^{32}.{Colors.RESET}")
                print(f"{Colors.CYAN}Math: \\Sigma_1(e) = ROTR^6(e) \\oplus ROTR^{11}(e) \\oplus ROTR^{25}(e), Ch(e,f,g) = (e \\land f) \\oplus (\\neg e \\land g), temp1 = h + \\Sigma_1(e) + Ch(e,f,g) + K_j + w_j mod 2^{32}.{Colors.RESET}")
                print(f"{Colors.CYAN}Math: Update: a' = temp1 + temp2 mod 2^{32}, e' = d + temp1 mod 2^{32}, then shift registers (h=g, g=f, f=e, e=e', d=c, c=b, b=a, a=a').{Colors.RESET}")
        ...
        if expanded:
            print(f"{Colors.CYAN}Math: Final update: H_i = H_i + working_var_i mod 2^{32} for i=0 to 7 (Davies-Meyer one-way compression).{Colors.RESET}")

    digest = b''.join(h.to_bytes(4, 'big') for h in H)
    if expanded:
        print(f"{Colors.CYAN}Math: Digest = H_0 || H_1 || ... || H_7 (concatenation, 256 bits). Security: Preimage resistance ~2^{256}, second preimage ~2^{256}, collision ~2^{128} (birthday paradox). No known attacks reduce this below brute-force in 2025.{Colors.RESET}")
    return digest

def dsha256_detailed(data: bytes, verbose: bool = False, expanded: bool = False) -> bytes:
    if expanded:
        print(f"{Colors.CYAN}Math (Double SHA-256): H = SHA256(SHA256(data)). In Bitcoin PoW: Interpret H (little-endian) as integer < target T, where T = 2^{256 - log2(difficulty)} (approx). Finding such nonce is ~2^{32} trials per block attempt on average.{Colors.RESET}")
