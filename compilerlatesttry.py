...
def setup_n64_toolchain():
    ...
    if platform.system() == "Darwin":
        ...
        try:
            subprocess.check_call(["brew", "tap", "messense/macos-cross-toolchains"])
            subprocess.check_call(["brew", "install", "mips64-elf-gcc@13"])
            # Set up symlinks in N64_INST for consistency with script expectations
            ...
            for tool in glob.glob(f"{brew_bin}/mips64-elf-*"):
    ...
    if not used_brew:
        ...
        steps = [
            ...
            {"name": "gcc-stage2", "dir": N64SDK_DIR / "gcc-14.2.0", "cfg": ["../configure", f"--prefix={N64_INST}", "--target=mips64-elf", "--enable-languages=c,c++", "--disable-multilib", "--with-newlib", "--disable-werror", "--with-abi=64", "--with-system-zlib", f"--with-gmp={N64_INST}", f"--with-mpfr={N64_INST}", f"--with-mpc={N64_INST}"]},
        ]
        ...
        for step in steps:
            ...
            if not configure_script.exists():
                ...
                sys.exit(1)
            if platform.system() == "Darwin" and step["name"] == "binutils-2.42":
            ...
            if step["name"] == "gcc-stage1":
                ...
                fixed_dir = N64_INST / "lib" / "gcc" / "mips64-elf" / "14.2.0" / "include"
                fixed_fixed_dir = N64_INST / "lib" / "gcc" / "mips64-elf" / "14.2.0" / "include-fixed"
                if fixed_dir.exists():
